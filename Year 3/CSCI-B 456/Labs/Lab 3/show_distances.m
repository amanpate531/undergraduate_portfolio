function show_distances(input_matrix, location_1, location_2, distance_measure_choice)
    ...
    y2 = location_2(1);
    result = 0;
    if (y1 > dim(1) || y1 <= 0 || x1 > dim(2) || x1 <= 0 || y2 > dim(1) || y2 <= 0 || x2 > dim(2) || x2 <= 0)
        disp("At least one of the given points is out of bounds");
    elseif distance_measure_choice == 0
        result = ((x1-x2) ^ 2 + (y1-y2) ^ 2) ^ 0.5;
    elseif distance_measure_choice == 1
        result = (abs(x1-x2) + abs(y1-y2));
    elseif distance_measure_choice == 2
...
function show_distance_connected(distance_measure_choice, connectivity)
    img = imread('iu.png');
    red = img(:, :, 1);
    green = img(:, :, 2);
    blue = img(:, :, 3);
    ...
    prompt1 = sprintf("\nEnter a number between 1 and %d: ", currentGroup);
    prompt2 = sprintf("\nEnter a different number between 1 and %d: ", currentGroup);
    group1 = input(prompt1);
    group2 = input(prompt2);
    point1 = zeros(1, 2);
    point2 = zeros(1, 2);
    for i = 1:dim(1)
        for j = 1:dim(2)
            if (resultingMatrix(i, j) == group1)
                point1(1) = i;
                point1(2) = j;
            end
            if (resultingMatrix(i, j) == group2)
                point2(1) = i;
                point2(2) = j;
            end
        end
    end
    show_distances(resultingMatrix, point1, point2, distance_measure_choice);
    end